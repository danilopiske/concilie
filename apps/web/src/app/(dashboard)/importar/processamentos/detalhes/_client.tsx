...
function StatusGeral({ status }: { status: string }) {
  const map = {
    concluido: { icon: <CheckCircle2 className="w-5 h-5 text-green-500" />, label: 'Concluído', cls: 'bg-green-50 text-green-700' },
    em_andamento: { icon: <Loader2 className="w-5 h-5 text-blue-500 animate-spin" />, label: 'Em Andamento', cls: 'bg-blue-50 text-blue-700' },
    com_erro: { icon: <XCircle className="w-5 h-5 text-red-500" />, label: 'Com Erro', cls: 'bg-red-50 text-red-700' },
    parcial: { icon: <Clock className="w-5 h-5 text-amber-500" />, label: 'Parcial', cls: 'bg-amber-50 text-amber-700' },
    sem_dados: { icon: <Clock className="w-5 h-5 text-gray-400" />, label: 'Sem Dados', cls: 'bg-gray-50 text-gray-600' },
  };
              ...
              <div className="flex items-center gap-3">
                {item.progress !== undefined && (
                  <div className="w-16 bg-gray-100 rounded-full h-1.5">
                    <div className="bg-[#1e3a8a] h-1.5 rounded-full" style={{ width: `${item.progress}%` }} />
                  </div>
                )}
...
export function ProcessamentoDetalhesClient({ processamentoId }: { processamentoId: string }) {
  const id = processamentoId;
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        <Link href="/importar/processamentos" className="p-2 rounded-lg hover:bg-gray-100 transition-colors">
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-red-400 text-sm">Não foi possível carregar os detalhes.</div>
      ) : detalhes ? (
        <>
          ...
          <ArquivosImportados processamentoId={id} />

          <TaskSection title="Importações" items={detalhes.importacoes} icon={Upload} emptyMsg="Nenhuma importação realizada" />
          <TaskSection title="Cálculos" items={detalhes.calculos} icon={Calculator} emptyMsg="Nenhum cálculo realizado" />
          <TaskSection title="Relatórios" items={detalhes.relatorios} icon={FileBarChart} emptyMsg="Nenhum relatório gerado" />
          <TaskSection title="Análise de Abusividade" items={detalhes.abusividades} icon={AlertTriangle} emptyMsg="Nenhuma análise de abusividade" />
        </>
      ) : null}
    </div>
  );
