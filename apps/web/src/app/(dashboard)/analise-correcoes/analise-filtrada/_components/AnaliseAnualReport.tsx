...
import { analistaFiltradaApi, AgregacaoFormaPagamentoAno } from '@/lib/api/analista';
...
export function AnaliseAnualReport({ processamentoId }: AnaliseAnualReportProps) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        const result = await analistaFiltradaApi.getFormasPorAno(processamentoId);
        setData(result);
      } catch (err) {
  ...
  return (
    <Card title="Formas de Pagamento por Ano — Filtradas">
      <Table
        columns={[
          { key: 'ano', label: 'Ano' },
